...
def runModels(depV,IDnames,groups, knownSigns, origDep,datadf):
    ...
    coef=np.reshape(mod1.coef_,-1,1)
    #join them
    intcoef=np.r_[int,coef]
...
def decomp0(X1,Y1,origDep,intcoef,depV,depMeans,transforms,rawdf,IDnames):
    ...
    plainidx=np.arange(start=modSpaceDecomp.index.values.min(),stop=modSpaceDecomp.index.values.max()+1,dtype='int')
...
def makeGroupedDecomp(origSpaceDecomp,groups,depV):
    ...
    plainidx=np.arange(start=origSpaceDecomp.index.values.min(),stop=origSpaceDecomp.index.values.max(),dtype='int')
...
def createDash(groupedDecomp,IDnames,rawdf,groups,elasts):
    ...
    listPeriods=[]
    for i in np.arange(0,len(aggGroupedDecomp.index.values)-start):
        if i+1<=numberInHalf:
